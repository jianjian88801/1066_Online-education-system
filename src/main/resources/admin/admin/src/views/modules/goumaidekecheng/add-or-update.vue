      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="学科" prop="xueke">
          <el-input v-model="ruleForm.xueke" 
              placeholder="学科" clearable  :readonly="ro.xueke"></el-input>
        </el-form-item>
        <div v-else>
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `goumaidekecheng/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.goumaidekechengCrossAddOrUpdateFlag = false;
                  this.parent.search();
